...
class Summary:
    ...
    def set_intro(self, heading_title = None, final_pass = False):
    ...
    def set_description_from_intro(self):
        ...
        if self.intro is None: self.set_intro(heading_title = 'General description', final_pass = True)
        self.data['general']['description'] = self.intro.description if self.intro is not None and self.intro.description not in ['', None] else input('Copy "General Description" from HTML page and paste here. If cannot for some reason, input "migration: not in database. Needs update":')
    ...
    def set_generated_by_from_intro(self):
        ...
        if self.intro is None: self.set_intro(heading_title = 'Written by Products', final_pass = True)
        self.data['general']['generated_by'] = self.intro.description if self.intro is not None and self.intro.description not in ['', None] else input('Copy "Generated by Product" from HTML page and paste here. If cannot for some reason, input "migration: not in database. Needs update":')
